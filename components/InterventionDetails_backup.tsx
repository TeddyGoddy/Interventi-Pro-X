...
const InterventionDetails: React.FC<InterventionDetailsProps> = (props) => {
    const { intervention, assets, onDeleteIntervention, isExpanded, onToggleExpansion } = props;
    ...
    const TabButton: React.FC<{tab: Tab, label: string, icon: React.ReactNode}> = ({ tab, label, icon }) => (
        <button 
            ...
            className={`group inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 whitespace-nowrap ${
                activeTab === tab 
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-full flex flex-col transition-all duration-300">
